...
sudo mkdir $MAIN_DIR/TM-web-terminal-0.0.1
PACKAGE_MAIN_DIR=$MAIN_DIR/TM-web-terminal-0.0.1

# Create debian package directories
sudo mkdir -p $PACKAGE_MAIN_DIR/DEBIAN
sudo mkdir -p $PACKAGE_MAIN_DIR/usr/local/TM/web-terminal

# Copy web-terminal files
sudo cp $MAIN_DIR/.eslintrc.cjs $PACKAGE_MAIN_DIR/usr/local/TM/web-terminal
sudo cp $MAIN_DIR/server.js $PACKAGE_MAIN_DIR/usr/local/TM/web-terminal
sudo cp $MAIN_DIR/package.json $PACKAGE_MAIN_DIR/usr/local/TM/web-terminal

cd $PACKAGE_MAIN_DIR/usr/local/TM/web-terminal
sudo chmod +x $PACKAGE_MAIN_DIR/usr/local/TM/web-terminal/server.js
...
sudo dpkg --info $MAIN_DIR/TM-web-terminal-0.0.1.deb
sudo dpkg --contents $MAIN_DIR/TM-web-terminal-0.0.1.deb

# Move debian package to dist folder
sudo mkdir -p $MAIN_DIR/dist
sudo mv $MAIN_DIR/TM-web-terminal-0.0.1.deb $MAIN_DIR/dist
