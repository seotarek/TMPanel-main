...
INSTALLER_URL="https://raw.githubusercontent.com/seotarek/TMPanel/main/installers/${DISTRO_NAME}/${DISTRO_VERSION}/install.sh"

INSTALLER_CONTENT=$(wget ${INSTALLER_URL} 2>&1)
if [[ "$INSTALLER_CONTENT" =~ 404\ Not\ Found ]]; then
    echo "TMPanel not supporting this version of distribution"
    echo "Distro: ${DISTRO_NAME} Version: ${DISTRO_VERSION}"
    echo "Exiting..."
    exit 1
fi

wget $INSTALLER_URL -O ./TM-installer.sh
chmod +x ./TM-installer.sh
bash ./TM-installer.sh
